...
Returns a data.frame with the first column as a character vector.

```{r}
cats[[1]]
```

The output is the first column as a character vector

```{r}
cats$coat
```

The output is the coat column as character vector
...
The output is the value of the first column of the first row

```{r}
cats[, 1]
```

The output is all the rows of the first column as a character vector.

```{r}
cats[1, ]
```

The output is a data.frame containing all variables (columns) for the first observation (row)
...
18. Yes, 6x3 = 18. The total number of entries.
...
```{r}
list(data_types = c('logical', 'integer', 'double', 'complex', 'character'), data_structures = c('vector', 'list', 'data.frame', 'matrix'))
```
