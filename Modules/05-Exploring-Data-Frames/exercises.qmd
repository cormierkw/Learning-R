...
Converting `human_age` to a factor stores values as categories with integer indices from 1 to 4. When converted back to a numeric vector, it pulled the category identifier instead of the values. Would first need to convert the factor into a character vector and then to a numeric.
...
```{r}
# create people
people <- data.frame(first_name = "Kevin", 
                     last_name = "Cormier", 
                     lucky_number = 7)
  
# add a team member
people <- rbind(people, data.frame(first_name = "Randy", 
                                   last_name = "Johnson", 
                                   lucky_number = 7))
...
#look at some random(ish) rows in the middle of the table
set.seed(234789)
...
We are working with a data.frame (tibble) with 1,704 rows and 6 columns:
...
gdpPercap is a numeric vector of length 1704.
